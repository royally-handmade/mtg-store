  ...
  <div class="scryfall-admin p-6">
    ...
    <div v-if="activeTab === 'single'" class="space-y-6">
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <!-- Import Result -->
        <div v-if="importResults.single" class="mt-4 p-4 rounded-md" :class="importResults.single.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'">
          <div class="flex">
      ...
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <div v-if="selectedSetInfo" class="mb-6 p-4 bg-gray-50 rounded-md">
          <div class="flex items-center space-x-4">
            <img v-if="selectedSetInfo.icon_svg_uri" :src="selectedSetInfo.icon_svg_uri" :alt="selectedSetInfo.name" class="w-8 h-8">
            <div>
              ...
              <p class="text-sm text-gray-600">
                ...
                {{ selectedSetInfo.imported_count }}/{{ selectedSetInfo.card_count }} imported ({{ selectedSetInfo.import_percentage }}%)
              </p>
            </div>
          </div>
        </div>
        ...
        <button
          @click="importSet"
          :disabled="!selectedSet || importing.set"
            ...
            <div class="mt-2">
              <div class="bg-blue-200 rounded-full h-2">
                <div 
                  ...
                  :style="{ width: setImportProgress.percentage + '%' }"
                ></div>
              </div>
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="mt-2 flex items-center justify-between">
                  ...
                  <div class="flex items-center space-x-2">
                    <span
                      v-if="card.is_imported"
                      class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800"
                    >
                      ✓ Imported
                    </span>
                    <button
                      @click="importCardFromSearch(card)"
                      :disabled="card.is_imported || importing.search"
...
export default {
  ...
  setup() {
    ...
    const importSet = async () => {
      ...
      try {
        const response = await fetch(`/api/scryfall/import-set`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          },
          body: JSON.stringify({ set_code: selectedSet.value })
        })
    ...
    const startBulkImport = async () => {
      ...
      try {
        const response = await fetch(`/api/scryfall/bulk-import`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
